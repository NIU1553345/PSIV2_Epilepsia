...
class CNN1D(nn.Module):
    def __init__(self, input_length, num_classes=2):
        ...
        if input_length == 128:     #Average 
            input_features = 64*3
        else:                       #Flatten
            input_features = 64*83
...
for i in range(-1,-5,-1):
        #leave one pacient out
    print(f"\n\n\n\nFold {(-1)*i}:")
    pacients_copia = parquet_pacients.copy()
    num_p = list(pacients_copia.keys())[i]
    metadata_out = pacients_copia[num_p]
    tensor_patient_out = npz_pacients[num_p]

    pacient_average_out = avg_pool(tensor_patient_out)

    tensor_class = torch.tensor(metadata_out['class'])
    test_dataset = TensorDataset(pacient_average_out , tensor_class)  # tensor_class ha de tenir valors 0 o 1

    pacients_copia.pop(num_p)    #Pacient de test  
    for num_p, metadata in pacients_copia.items():
        ...
        train_dataset = TensorDataset(pacient_average , tensor_class)  # tensor_class ha de tenir valors 0 o 1
    ...
    print("Primeras 100 prediccions:", predictions[:100])
